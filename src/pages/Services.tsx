  ...
  const servicesData = {
    fire_suppression: {
      ...
      metrics: [
        ...
        { label: language === 'en' ? 'Response Time' : 'سرعة الاستجابة', value: '< 10 Seconds' },
        { label: language === 'en' ? 'Pipe Lifespan' : 'عمر الشبكات الافتراضي', value: '25+ Years' },
      ],
      color: 'red',
      img: 'https://images.unsplash.com/photo-1594824476967-48c8b964273f?w=600&fit=crop'
    },
    fire_detection: {
      title: language === 'en' ? 'Intelligent Detection & Alarms' : 'أنظمة الإنذار والتوجيه الفوري',
      subtitle: language === 'en' ? 'Advanced sensors, smoke controllers, and voice announcement systems' : 'كواشف ذكية موجهة وأنظمة إنسحاب معالجة وحساسة',
      icon: <BellRing className="w-6 h-6 text-amber-500" />,
      features: [
        language === 'en' ? 'Analog addressable fire safety command systems.' : 'تركيب وتجهيز لوحات إنذار حريق ذكية موجهة ومعنونة.',
        language === 'en' ? 'Aspiration smoke sensors and early warning air sampling (VESDA).' : 'أجهزة استنشاق الهواء الحساسة للكشف الفوري عن الدخان (VESDA).',
        ...
        language === 'en' ? 'Intelligent gas detection (CO, Methane, Hydrogen) for hazardous plants.' : 'كواشف غازات سامة وقابلة للاشتعال للمصانع والمنشآت البترولية.'
      ],
      metrics: [
        { label: language === 'en' ? 'Accreditation' : 'الاعتماد الكودي', value: 'NFPA 72 & UL Listed' },
        { label: language === 'en' ? 'False Alarms Rate' : 'معدل الإنذارات الخاطئة', value: '0.001%' },
        { label: language === 'en' ? 'Battery Backup' : 'بطاريات الطوارئ الاحتياطية', value: '72 Hours' },
      ],
      color: 'amber',
      img: 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=600&fit=crop'
    },
    hvac: {
      title: language === 'en' ? 'Air Conditioning & Cooling (HVAC)' : 'أنظمة التكييف والتهوية وتبريد المجمعات',
      subtitle: language === 'en' ? 'Designing premium air control, heating, and cooling networks' : 'الأعمال المتكاملة لتكييف الهواء المركزي، أنظمة الحجم المتغير، والتشيلر',
      icon: <Wind className="w-6 h-6 text-blue-500" />,
      features: [
        language === 'en' ? 'Modular air-cooled and water-cooled chillers.' : 'مبردات المياه تشيلر (Water & Air Cooled) للمباني الشاهقة.',
        language === 'en' ? 'Variable Refrigerant Flow (VRF) systems for custom room temp management.' : 'أنظمة التكييف ذات التدفق المتغير (VRF) لضبط غرف مستقلة موفرة للطاقة.',
        language === 'en' ? 'Custom duct design, fabrication, and insulation in GCC certified factories.' : 'تفصيل وتصنيع وعزل مجاري الهواء (Duct) بمصانعنا المتطورة.',
        language === 'en' ? 'Industrial dust collection and mechanical ventilation systems.' : 'أنظمة تبريد المصانع، سحب الأبخرة، وسحب الدخان المركزي.',
        language === 'en' ? 'Direct Digital Control (DDC) and Smart thermostat integration.' : 'أنظمة DDC للتحكم الرقمي المباشر وتكامل غرف التحكم الذكية.'
      ],
      metrics: [
    ...
    cctv_security: {
      ...
      features: [
        language === 'en' ? 'Ultra-HD IP security cameras with night vision and smart AI tracking.' : 'كاميرات المراقبة بجودة فائقة (Ultra-HD IP) مع الرؤية الليلية والتتبع الذكي.',
        language === 'en' ? 'Biometric access control, card readers, and turnstiles.' : 'أنظمة التحكم بالبوابات البيومترية، قارئات الكروت، والبوابات الدوارة الذكية.',
      ...
      metrics: [
        ...
        { label: language === 'en' ? 'AI Video Analytics' : 'تحليل الفيديو الذكي', value: 'Face & license Plate' },
      ],
      color: 'emerald',
